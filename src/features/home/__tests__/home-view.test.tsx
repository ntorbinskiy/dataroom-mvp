...
function stubProps(overrides: Partial<HomeViewProps> = {}): HomeViewProps {
...
describe('HomeView (pure view over HomeViewProps)', () => {
  it('renders the empty state when there are no rooms', () => {
    renderWithProviders(<HomeView {...stubProps()} />)
  ...
  it('renders room cards with display-ready meta from the contract', () => {
    ...
    const props = stubProps({ rooms: [room], roomMeta: new Map([['r1', '3 files · 12 MB · Jul 12']]) })
    renderWithProviders(<HomeView {...props} />)
  ...
  it('renders the error state with retry', () => {
    const retry = vi.fn()
    renderWithProviders(<HomeView {...stubProps({ isError: true, retry })} />)
