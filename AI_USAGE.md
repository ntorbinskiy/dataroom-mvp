...
- Task 1 - Scaffold, Makefile, git init (57d8f16). Vite + React + TypeScript +
  Vitest scaffold, jsdom test environment. Review: clean; minor note (no
  `engines` field in package.json) left as a non-blocker.
- Task 2 - Design tokens, fonts, shadcn/ui kit (a18bc1a). Tailwind design
...
- Task 3 - Domain types + naming module, TDD (d2854d6). `core/types.ts`,
  `core/naming.ts` (duplicate-name auto-suffix resolution). Review: clean;
  minor gap noted (no multi-dot filename test), not blocking.
- Task 4 - format + upload validation helpers, TDD (6195bda). Byte/date
...
- Task 5 - Repository contract, in-memory adapter, shared contract suite, TDD
  (7b2d4e5, fix 270c595). `DataroomRepository` port, `MemoryRepository`, an
...
- Task 6 - IndexedDB adapter passing the same contract suite, TDD (e4a1a6a).
...
- Task 7 - DI (RepositoryProvider) + domain hooks over the port (1757405).
...
- Task 8 - App shell, routing, feedback components, test utils (a45e3ae).
  TopBar, EmptyState/ErrorState/TableSkeleton, routed page shells,
  `renderWithProviders` test helper. Review: clean.
- Task 9 - Shared dialogs: NameDialog + DeleteConfirmDialog, TDD (4ab3a56, fix
  42333b6). Review: 1 fix round - centralized React Testing Library `cleanup()`
...
- Task 10 - Home page: props contract, page hook, pure view, container
  (79681b3). Review: clean; minors queued for the polish pass (ASCII `...`
...
- Task 11 - Folder view: props contract, page hook, node components, pure
  view, container (6353111). Breadcrumbs, NodeTable, upload button and
...
- Task 12 - File viewer: props contract, page hook, pure view, container
  (c835d4d). iframe-based PDF viewer, object-URL lifecycle. Review: clean;
  minor queued (`createObjectURL` inside `useMemo`, a StrictMode dev-only
  leak).
- Task 13 - Polish pass + end-to-end verification (fix cae9fd1, fix ba73de1).
  ...
  favicon, commit ba73de1).
- Task 14 - README, AI usage log, vercel.json, .claude gitignore (866449e).
  Review: clean.
- 2026-07-18 - Feature-folder refactor + `.port.ts` naming convention
  (d6565cf). A design discussion (colocation argument: a page's contract,
...
- 2026-07-19 - Vocabulary unification (6af27b5): ViewModel/vm removed in favor
...
- 2026-07-19 - UX dedup (c3aff6c) and narrow-viewport fix (50d4963): one
